...
/**
 * Print The Shot Plugin Dev Server
 *
 ...
 *
 * Usage:
 *   npm run serve                          # defaults: port 4445, bridge at localhost:8080
...
const PLUGIN_PATH = resolve(
  __dirname,
  "../../assets/plugins/print-the-shot.reaplugin/plugin.js"
);
...
<html><head><title>Print The Shot Dev Server</title>
...
<body><h1>🖨️ Print The Shot Dev Server</h1>
...
try {
  ...
  console.error(
    "Build it first: cd packages/print-the-shot-plugin && npm run build"
  );
  process.exit(1);
}
...
server.listen(PORT, () => {
  console.log(`\nPrint The Shot Dev Server running at http://localhost:${PORT}`);
